...
export const LinqAccountConfigSchema: z.ZodType<Record<string, unknown>> = z.lazy(() =>
  z
    .object({
      ...
      apiTokenRef: secretRefSchema.optional(),
      tokenFile: z.string().min(1).optional(),
      fromPhone: e164PhoneSchema.optional(),
      dmPolicy: z.enum(["pairing", "open", "disabled"]).default("pairing").optional(),
    ...
    .superRefine((value, ctx) => {
      const tokenSources = [value.apiToken, value.apiTokenRef, value.tokenFile].filter(Boolean);
      if (tokenSources.length > 1) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "configure only one of apiToken, apiTokenRef, or tokenFile",
          path: ["apiToken"],
        });
      }
    }),
...
export const LinqConfigJsonSchema = {
  ...
  properties: {
    ...
    apiTokenRef: { $ref: "#/$defs/secretRef" },
    tokenFile: { type: "string", minLength: 1 },
    fromPhone: { type: "string", pattern: "^\\+[1-9]\\d{6,14}$" },
    dmPolicy: { enum: ["pairing", "open", "disabled"], default: "pairing" },
    ...
    webhookSecretRef: { $ref: "#/$defs/secretRef" },
